...
from DTforCurrency_2split import constant
...
def calcDistancesMetric_2d(dataType, distanceMeasure, X):
    length = len(X)
    #生成一个长宽为length的二维矩阵
    distances = [[.0] * length for _ in range(length)]
    for j in range(length):
        for i in range(j+1):
            disTuple = (i,j,calcDistance(dataType, distanceMeasure, X[i], X[j]))
            distances[i][j] = disTuple
            distances[j][i] = disTuple
    return distances
...
def getDistance(file,index1, index2):
    gl_distances = constant.get_value('gl_distances_'+file)
    i = min(index1, index2)
    j = max(index1, index2)
    if i == j:
        dis = 0
...
def getGiniIndex(file,Dataset):
    gl_ytrain = constant.get_value('gl_ytrain_'+file)
...
def checkPartition(file,dataset, cate):
    # 如果数据集中的数据都相同，则返回该数据集的类别，其类别由出现次数最多的y决定。否则返回None
    xSet = constant.get_value('gl_Xtrain_'+file)[dataset]
    length = len(xSet)
    ...
    return np.argmax(np.bincount(constant.get_value('gl_ytrain_'+file)[dataset].astype(int)))
    # return cate
...
def getMean(file,dataset_cate_index):
    gl_Xtrain = constant.get_value('gl_Xtrain_'+file)
...
def getMode(file,dataset_cate_index):
    gl_Xtrain = constant.get_value('gl_Xtrain_'+file)
    dataset_cate = np.array([gl_Xtrain[index] for index in dataset_cate_index])
    mode = []
    for row in range(len(dataset_cate[0])):
